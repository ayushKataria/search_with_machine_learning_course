...
class DataPrepper:
        ...
        for key in query_gb.groups.keys():
            ...
            try:
                ...
                if response and response['hits']['hits'] and len(response['hits']['hits']) > 0:
                    ...
                    # print(hits)
                    skus_for_query = prior_clicks_for_query[["sku","query","clicks","num_impressions"]][prior_clicks_for_query["query"] == key].drop_duplicates()  # we are comparing skus later, so grab the Series now
                    ...
                    for (idx, hit) in enumerate(hits):
                        ...
                        num_impressions.append(skus_for_query[skus_for_query["sku"] == sku]["num_impressions"].sum())
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        for hit in log_query_res["hits"]["hits"]:
            doc_id = hit["_source"]["sku"]
            feature_results["doc_id"].append(doc_id)  # capture the doc id so we can join later
            feature_results["query_id"].append(query_id)
            feature_results["sku"].append(doc_id)  # ^^^
            for value in hit["fields"]["_ltrlog"][0]["log_entry"]:
                if value["name"] in feature_results:
                    feature_results[value['name']].append(value.get("value", 0))
                else:
                    feature_results[value['name']] = [value.get("value", 0)]
    ...
    def __num_clicks(self, all_skus_for_query, test_sku):
        return all_skus_for_query[all_skus_for_query["sku"] == test_sku]["clicks"].sum()
